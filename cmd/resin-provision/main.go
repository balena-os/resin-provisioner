...
this option if you need to create a new account or application.`,
	...
	rootCmd.PersistentFlags().StringVarP(&domain, "domain", "d", provisioner.DEFAULT_RESIN_DOMAIN, "Domain the device will provision to")
	rootCmd.PersistentFlags().StringVarP(&configPath, "config", "c", provisioner.CONFIG_PATH, "Supervisor config path")
...
func oneshot(cmd *cobra.Command, args []string) error {
	if state, err := provisioner.New(configPath).State(); err != nil {
		return err
	} else if state == provisioner.Provisioning || state == provisioner.Provisioned {
		return nil
	} else if token == "" {
	...
	if appId, err := getApp(token, application); err != nil {
		...
		return provision(appId, apiKey, userId, userName, configPath, dryRun)
	}
}
...
func provision(appId, apiKey, userId, userName, configPath string, dryRun bool) error {
	...
	if err := provisioner.New(configPath).Provision(opts); err != nil {
		return err
	}
...
func success(cmd *cobra.Command, args []string) error {
	if !dryRun {
		if url, err := provisioner.New(configPath).DeviceUrl(); err == nil {
